...
export function Sidebar({ active, setPage, user, compactOnMobile = true }) {
  const [collapsed, setCollapsed] = useState(() => compactOnMobile && window.matchMedia('(max-width: 700px)').matches);
...
// ─── Layout wrapper ─────────────────────────────────────────────────────
export function AppLayout({ active, setPage, setSelectedPatient, onLogout, user, children, compactOnMobile = true }) {
  return (
    <div
      className="app-shell"
      style={{
    ...
    >
      <GlobalOverlay />
      <Sidebar active={active} setPage={setPage} user={user} compactOnMobile={compactOnMobile} />
      <div
        style={{
          flex: 1,
          display: "flex",
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto" }} className="page-transition app-content">{children}</div>
      </div>
    </div>
  );
}
